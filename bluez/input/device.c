...
static gboolean/*void*/ extract_hid_record(sdp_record_t *rec, struct hidp_connadd_req *req)
{
	sdp_data_t *pdlist, *pdlist2;
	uint8_t attr_val;
	// qual: PTS_HCE_BV_04_I - to handle auto reconnect
	uint8_t  normally_connectable;
	uint8_t reconnect_initiated;
	//END qual
...
// qual: PTS_HCE_BV_04_I - to handle auto reconnect
	...
	reconnect_initiated = pdlist ? pdlist->val.uint8 : 0;

	pdlist = sdp_data_get(rec, SDP_ATTR_HID_NORMALLY_CONNECTABLE);
	normally_connectable = pdlist ? pdlist->val.uint8 : 0;

	error("**** extract_hid_record	reconnect_initiated: %d and normally_connectable : %d",
				reconnect_initiated,normally_connectable);
// qual END
...
// qual: PTS_HCE_BV_04_I - to handle auto reconnect
	if(normally_connectable && !reconnect_initiated)
		return 1;
	else
		return 0;
// qual END
...
// qual: TP/HCR/BV-03-I - Host initiated virtual cable unplug
    /*original code
	err = disconnect(idev, 0);
    */
     //neeraj_blocked qcom code for Recoonect after booting problem
