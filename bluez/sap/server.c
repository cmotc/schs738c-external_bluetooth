...
{
	gsize written = 0;//edited to remove warning message regarding TYPE
	GError *gerr = NULL;
	GIOStatus gstatus;
	...
	if (((size_t)written) != size)
		error("write error.(written %zu size %zu)", (size_t)written, size);//edited to remove warning message regarding TYPE
								...
								void *data)
{
	...
	if (disconnect_req(server->conn, SAP_DISCONNECTION_TYPE_IMMEDIATE) < 0)
		return g_dbus_create_error(msg, ERROR_INTERFACE	".Failed",
				"There is no active connection");
